    ...
    <div class="flex column PlanManagement flex-1 ">
        ...
        <div class="flex flex-1 column ">
            <table class="column width flex-end status">
                <tr>
                    <th>计划编号: {{route.id}}</th>
                    <th>计划类型: {{route.type}}</th>
                    <th>生成时间: {{route.createTime}}</th>
                </tr>
                <tr>
                    <th>计划状态:{{route.status}}</th>
                    <th>中奖情况:{{route.winStatus}}</th>
                    <th>中奖金额: {{route.income}}</th>
                </tr>
            </table>
        </div>
        <h4>计划明细</h4>
        ...
        <table class="column width ">
            ...
            <template v-for="(item, index) in follow" :key="index">
                <tr>
                    <td>{{index}}</td>
                    <td>{{item.nickname}}</td>
                    <td>{{item.time}}</td>
                    ...
                    <td>
                       <a href="#" @click="jump('/Notes' ,item.followId)">查看笔记</a>
                    </td>
                </tr>
            </template>
        </table>
...
<script>
    ...
    export default defineComponent({
        ...
        setup() {
            ...
            const Prepare = (eve) => {
                state.data.url = '/project/football/' + route.id
                new Promise((resolve) => {
                    return resolve(proxy.$get(state.data))
                }).then((res) => {
                    if (res.data.code == 0) {
                        state.rows = res.data.data.details
                        state.follow = res.data.data.follow
                    } else {
